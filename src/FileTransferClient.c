...
int main(int argc, char *argv[]){
   ...
   /* If there are not arguments print an information message           */
	...
	if(validate_address(host_addr)==4){
		...
		inet_pton(AF_INET6,host_addr,&server6.sin6_addr);
		...
		if(connect(sock_desc, (struct sockaddr *)&server6, sizeof(server6))<0){
			printf("connection failed");
			return -1;
		}
	}else{
	...
	sscanf(recv_buf,"FILE SIZE IS %d",&file_size);
	printf("file size:%d\n",file_size);

	//printf(recv_buf);
	
	if((outfile = fopen(requested_file,"w"))==NULL){
		fprintf(stderr,"COULD NOT OPEN OUTPUT FILE\n");
		close(sock_desc);
		exit(0);
	}
	...
	do{
		nread = read(sock_desc,recv_buf,buf_size);
		if( ( nread  < 0) ){
			fprintf(stderr,"error reading from the socket");
			close(sock_desc);
